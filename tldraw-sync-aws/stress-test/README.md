# Stress Test for tldraw-sync-aws
...
- Run tests from inside the same cloud/region as the deployment (Cloud Shell/VM, or an EC2 instance) for accurate results
...
3. **Prompt you** to scale UP to 5 replicas (via kubectl)
...
| `BASE_URL`            | `wss://aws-sync.example.com` | Target server           |
...
docker run --rm -i \
  -v $(pwd):/scripts \
  grafana/k6 run \
  -e BASE_URL=wss://aws-sync.example.com \
  -e ROOMS=10 \
  -e USERS_PER_ROOM=10 \
  -e DURATION=1m \
...
## Running from inside the cloud network (Recommended)

For accurate results, run stress tests from inside the deployment's network (an EC2 instance in the same VPC, or GCP Cloud Shell for the GCP demo). Some shells have permission issues with volume mounts, so use stdin:
...
const BASE_URL = __ENV.BASE_URL || 'wss://aws-sync.example.com';
...
This tests 7,000 VUs (100 rooms × 70 users) from inside the deployment's network.
