...
4. **Owner prepares**: Pod A saves state to S3, releases lock
...
7. **Load state**: Pod C loads room state from S3
...
To prevent load balancer idle timeouts from killing WebSocket connections, the server implements server-side ping. The 25s interval was tuned for GCP's 30s LB idle timeout; on AWS it comfortably clears both the ALB default (60s) and the NLB default (350s), so it is kept unchanged for parity with the GCP demo:

```typescript
const PING_INTERVAL_MS = 25000 // 25 seconds, under every LB idle timeout we target
...
1. **State is always saved before lock release**: S3 snapshot persisted in Phase 1
...
4. **No data loss**: Users reconnect to fresh state from S3
